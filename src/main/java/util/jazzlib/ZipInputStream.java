...
public class ZipInputStream extends InflaterInputStream implements ZipConstants {
	...
	private int readBuf(final byte[] out, final int offset, int length)
			throws IOException {
		if (avail <= 0) {
			fillBuf();
			if (avail <= 0) {
	...
	private void readFully(final byte[] out) throws IOException {
		int off = 0;
		int len = out.length;
		while (len > 0) {
			final int count = readBuf(out, off, len);
			if (count == -1) {
	...
	public ZipEntry getNextEntry() throws IOException {
		...
		flags = readLeShort();
		method = readLeShort();
		final int dostime = readLeInt();
		final int crc = readLeInt();
		csize = readLeInt();
		size = readLeInt();
		final int nameLen = readLeShort();
		final int extraLen = readLeShort();

		if ((method == ZipOutputStream.STORED) && (csize != size)) {
			throw new ZipException("Stored, but compressed != uncompressed");
		}

		final byte[] buffer = new byte[nameLen];
		readFully(buffer);
	...
	public void closeEntry() throws IOException {
		...
		if ((avail > csize) && (csize >= 0)) {
			...
			avail = 0;
			while (csize != 0) {
				final long skipped = in.skip(csize & 0xffffffffL);
				if (skipped <= 0) {
	...
	public int read(final byte[] b, final int off, int len) throws IOException {
		...
		if (finished) {
			final long entryCrc = entry.getCrc();
			if ((entryCrc >= 0) && ((crc.getValue() & 0xffffffffL) != entryCrc)) {
