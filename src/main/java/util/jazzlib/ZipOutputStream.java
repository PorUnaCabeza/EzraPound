...
ZipConstants {
	 ...
	 */
	private final void writeLeShort(final int value) throws IOException {
	 ...
	 */
	private final void writeLeInt(final int value) throws IOException {
		writeLeShort(value);
	 ...
	 * @exception net.sf.jazzlib.ZipException
	...
	public void putNextEntry(final ZipEntry entry) throws IOException {
		...
		final byte[] name = entry.getName().getBytes();
	...
	public void closeEntry() throws IOException {
		...
		final int csize = curMethod == DEFLATED ? def.getTotalOut() : size;
		...
		if (curEntry.getCrc() < 0) {
			...
			throw new ZipException("crc was "
					+ Long.toHexString(crc.getValue()) + ", but I expected "
	...
	public void finish() throws IOException {
		...
		final Enumeration elements = entries.elements();
		while (elements.hasMoreElements()) {
			final ZipEntry entry = (ZipEntry) elements.nextElement();
			...
			final String strComment = entry.getComment();
			final byte[] comment = strComment != null ? strComment.getBytes()
					: new byte[0];
